...
interface StatusBookingsPageProps {
  ...
  approvalType?: "sales" | "accountant";
}

export default function StatusBookingsPage({
  status,
  title,
  subtitle,
  emptyMessage,
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await getVendorBookings(token, { page, limit: 20, status, salesApproved, accountantApproved, search: search || undefined });
  ...
  }, [token, page, search, status, salesApproved, accountantApproved]);

  useEffect(() => { fetchData(); }, [fetchData]);

  const handleApprove = async (bookingId: string) => {
    if (!token || !approvalType) return;
    setApproving(bookingId);
    try {
      if (approvalType === "sales") {
        await approveSales(token, bookingId);
      } else {
        await approveAccountant(token, bookingId);
      }
      fetchData();
    } catch (e: any) {
  ...
  const columns: Column<VendorBooking>[] = [
    ...
    ...(approvalType ? [{
      ...
      accessor: (row: VendorBooking) => (
        ...
        >
          {approving === row.id ? "Approving..." : approvalType === "sales" ? "Sales Approve" : "Accountant Approve"}
        </button>
      ),
  ...
  const resolvedSubtitle = subtitle
    ? subtitle.replace("{count}", String(bookings.length))
