...
export default function FinishedBookingsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await getVendorBookings(token, { page, limit: 20, status: "COMPLETED", search: search || undefined });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Finished Bookings</h1>
      ...
      <DataTable
        ...
        exportable
        exportFilename="COMPLETED-bookings"
        exportTitle="Finished Bookings"
        columnVisibility
        emptyMessage="No finished bookings"
      />
      {totalPages > 1 && (
