...
import { getVendorBookings, approveAccountant } from "../../../../lib/vendor";
...
export default function AccountantPendingPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await getVendorBookings(token, { page, limit: 20, status: "PENDING_APPROVAL", accountantApproved: false, search: search || undefined });
  ...
  const columns: Column<VendorBooking>[] = [
    ...
    },
    {
      ...
      accessor: (row) => (
        ...
        >
          {approving === row.id ? "Approving..." : "Accountant Approve"}
        </button>
      ),
      align: "right",
    },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Accountant Pending</h1>
        <p className="text-sm text-slate-500 mt-1">Bookings awaiting accountant approval ({bookings.length} found)</p>
      </div>
      <DataTable
        columns={columns}
        data={bookings}
