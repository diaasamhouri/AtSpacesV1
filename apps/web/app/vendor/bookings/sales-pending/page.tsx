...
import { getVendorBookings, approveSales } from "../../../../lib/vendor";
...
export default function SalesPendingPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await getVendorBookings(token, { page, limit: 20, status: "PENDING_APPROVAL", salesApproved: false, search: search || undefined });
  ...
  const columns: Column<VendorBooking>[] = [
    ...
    },
    {
      ...
      accessor: (row) => (
        ...
        >
          {approving === row.id ? "Approving..." : "Sales Approve"}
        </button>
      ),
      align: "right",
    },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Sales Pending</h1>
        <p className="text-sm text-slate-500 mt-1">Bookings awaiting sales approval ({bookings.length} found)</p>
      </div>
      <DataTable
        columns={columns}
        data={bookings}
