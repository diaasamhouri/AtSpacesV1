...
export default function NoShowBookingsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await getVendorBookings(token, { page, limit: 20, status: "NO_SHOW", search: search || undefined });
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        exportable
        exportFilename="NO_SHOW-bookings"
        exportTitle="No Show Bookings"
        columnVisibility
        emptyMessage="No no-show bookings"
      />
      {totalPages > 1 && (
