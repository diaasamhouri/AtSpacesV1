...
export default function BookingOverviewPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await getVendorBookings(token, { page, limit: 20, search: search || undefined });
  ...
  const columns: Column<VendorBooking>[] = [
    ...
    },
    {
      header: "Status",
      accessor: (row) => (
        <span className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-bold ${STATUS_COLORS[row.status] || "bg-slate-500/10 text-slate-400 border border-slate-500/20"}`}>
          {formatStatusLabel(row.status)}
        </span>
      ),
      sortable: true,
      sortKey: "status",
      exportAccessor: (row) => formatStatusLabel(row.status),
    },
    {
      header: "Date",
      accessor: (row) => (
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        exportable
        exportFilename="overview-bookings"
