...
export default function VendorCalendarPage() {
    ...
    if (loading) {
        return <div className="p-8 text-center text-slate-500 dark:text-slate-400">Loading calendar...</div>;
    }
    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                {branches.length > 1 && (
                    <select
                        ...
                        className="rounded-xl border border-slate-200 dark:border-slate-700 bg-dark-900 px-4 py-2.5 text-sm font-bold text-gray-900 dark:text-white focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-colors"
                    >
            ...
            {events.length === 0 ? (
                <div className="rounded-2xl bg-dark-900 border border-dashed border-slate-200 dark:border-slate-700 p-12 text-center">
                    ...
                    <div className="md:hidden space-y-6">
                        {(() => {
                            ...
                            return Object.entries(grouped).map(([dateLabel, dayEvents]) => (
                                <div key={dateLabel}>
                                    <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-3 sticky top-0 bg-dark-950 py-1">{dateLabel}</h3>
                                    <div className="space-y-3">
                                        {dayEvents.map((ev) => (
                                            <div key={ev.id} className="rounded-2xl bg-dark-900 border border-slate-200 dark:border-slate-800 p-4">
