...
const ROLE_COLORS: Record<string, string> = {
  ...
  CUSTOMER: "bg-slate-500/10 text-slate-400 border border-slate-500/20",
};
    ...
    <div className="space-y-6">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-800">
              <thead className="bg-dark-850">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-slate-400">Name & Contact</th>
                  ...
                  {isSuperAdmin && <th className="px-6 py-4 text-right text-xs font-bold uppercase tracking-wider text-slate-400">Actions</th>}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/50">
                {users.length === 0 ? (
                  ...
                  <tr key={u.id} className="hover:bg-dark-850 transition-colors">
                    <td className="px-6 py-4">
                      <div className="text-sm font-bold text-white">{u.name || "Anonymous"}</div>
                      <div className="text-xs font-medium text-slate-400 mt-0.5">{u.email || u.phone || "\u2014"}</div>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider ${ROLE_COLORS[u.role]}`}>{u.role}</span>
                    </td>
                    <td className="px-6 py-4 text-sm font-medium text-slate-300">{format(new Date(u.createdAt), "MMM d, yyyy")}</td>
                    <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right text-sm">
                        {u.role !== "ADMIN" && (
                          <button
                            ...
                            className={`rounded-lg px-4 py-2 text-xs font-bold transition-all ${u.isActive ? "bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500/20" : "bg-green-500/10 text-green-500 border border-green-500/20 hover:bg-green-500/20"}`}
                          >
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowCreate(false)}>
          <div className="w-full max-w-md rounded-3xl bg-dark-900 p-8 shadow-2xl border border-slate-800 shadow-black/50" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-2xl font-bold text-white mb-6">Create Team User</h3>
            ...
            <div className="space-y-5">
              <div>
                ...
                <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  className="block w-full rounded-2xl border border-slate-700 px-4 py-3 text-sm font-medium text-white bg-dark-850 focus:bg-dark-900 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-colors" />
              </div>
              <div>
            ...
            <div className="mt-8 flex justify-end gap-4">
              <button onClick={() => setShowCreate(false)} className="rounded-xl border border-slate-700 px-6 py-3 text-sm font-bold text-white bg-dark-850 hover:bg-dark-800 hover:border-slate-600 transition-colors">Cancel</button>
