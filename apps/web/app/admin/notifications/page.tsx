...
import { getAdminNotifications, sendNotification, getAdminUsers } from "../../../lib/admin";
        ...
        <div className="space-y-6">
            ...
            {loading ? (
                ...
                <div className="space-y-2">
                    {notifications.map((n) => (
                        <div key={n.id} className={`rounded-2xl p-4 shadow-float border border-slate-800 transition-colors ${n.isRead ? "bg-dark-900" : "bg-dark-900 border-l-4 border-l-brand-500"}`}>
                            <div className="flex items-start justify-between">
                                <div>
                                    ...
                                    <div className="flex items-center gap-3 mt-2">
                                        ...
                                        <span className="text-xs text-slate-400">&rarr; {n.user?.name || n.user?.email || "\u2014"}</span>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowSend(false)}>
                    <div className="w-full max-w-md rounded-3xl bg-dark-900 p-8 shadow-2xl border border-slate-800" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-bold text-white mb-2">Recipient</label>
                                <select value={form.userId} onChange={(e) => setForm({ ...form, userId: e.target.value })}
                                    className="block w-full rounded-xl border border-slate-700 px-4 py-3 text-sm text-white bg-dark-850 focus:bg-dark-900 focus:border-brand-500 focus:ring-brand-500 transition-colors">
                            ...
                            <div>
                                ...
                                <textarea rows={3} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })}
                                    className="block w-full rounded-xl border border-slate-700 px-4 py-3 text-sm text-white bg-dark-850 focus:bg-dark-900 focus:border-brand-500 focus:ring-brand-500 transition-colors resize-none"
                                    placeholder="Write your message..." />
                            </div>
                        </div>
                        <div className="mt-6 flex justify-end gap-3">
                            <button onClick={() => setShowSend(false)} className="rounded-xl border border-slate-700 px-6 py-3 text-sm font-bold text-white bg-dark-850 hover:bg-dark-800 transition-colors">Cancel</button>
