        ...
        <div className="space-y-6">
            ...
            {loading ? (
                ...
                <>
                    {/* Pending */}
                    <div>
                        <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4">Pending ({pending.length})</h2>
                        {pending.length === 0 ? (
                            ...
                            <div className="space-y-4">
                                {pending.map((r) => (
                                    <div key={r.id} className={`rounded-2xl bg-dark-900 p-6 border border-slate-800 shadow-float border-l-4 ${TYPE_ACCENT[r.type] || "border-l-slate-500"}`}>
                                        <div className="flex items-start justify-between">
                                            <div>
                                                <StatusBadge status={r.type} label={TYPE_LABELS[r.type] || r.type} />
                                                <h3 className="text-lg font-bold text-white mt-3">{r.branch} <span className="text-slate-500 mx-1">&mdash;</span> {r.vendor}</h3>
                                                <p className="text-sm font-medium text-slate-300 mt-2 leading-relaxed">{r.description}</p>
                    ...
                    {processed.length > 0 && (
                        <div className="pt-8">
                            <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4">History ({processed.length})</h2>
                                    ...
                                    <table className="min-w-full divide-y divide-slate-800">
                                        <thead className="bg-dark-850">
                                            <tr>
                                                ...
                                                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-slate-400">Branch / Vendor</th>
                                        ...
                                        <tbody className="divide-y divide-slate-800/50">
                                            {processed.map((r) => (
                                                <tr key={r.id} className="even:bg-dark-850/30 hover:bg-dark-850/60 transition-colors">
                                                    <td className="px-6 py-4 text-sm font-bold text-white">{TYPE_LABELS[r.type] || r.type}</td>
                                                    <td className="px-6 py-4 text-sm font-medium text-slate-300">{r.branch} <span className="text-slate-500 mx-1">&mdash;</span> {r.vendor}</td>
                                                    <td className="px-6 py-4">
                                                        <StatusBadge status={r.status} />
                                                    </td>
                                                    <td className="px-6 py-4 text-sm font-medium text-slate-400">{format(new Date(r.createdAt), "MMM d, yyyy")}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
            ...
            {rejectId && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setRejectId(null)}>
                    <div className="w-full max-w-md rounded-3xl bg-dark-900 p-8 shadow-2xl border border-slate-800" onClick={(e) => e.stopPropagation()}>
                        ...
                        <p className="text-sm font-medium text-slate-400 mb-6">Please provide a reason for rejecting this request.</p>
                        ...
                        <div className="mt-6 flex justify-end gap-4">
                            <button onClick={() => setRejectId(null)} className="rounded-xl border border-slate-700 px-6 py-3 text-sm font-bold text-white bg-dark-850 hover:bg-dark-800 hover:border-slate-600 transition-colors">Cancel</button>
