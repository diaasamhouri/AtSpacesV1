...
export default function AdminApprovalsPage() {
    ...
    return (
        <div className="space-y-6">
            ...
            <SearchBar defaultValue={search} placeholder="Search by branch or vendor name..." />

            <div className="flex flex-wrap gap-2">
                {STATUS_FILTERS.map((s) => (
                    <button key={s} onClick={() => setStatusFilter(s)}
                        className={`rounded-xl px-4 py-2 text-xs font-bold transition-all duration-300 ${statusFilter === s ? "bg-brand-500 text-white shadow-[0_2px_8px_rgba(255,91,4,0.4)]" : "bg-slate-100 dark:bg-dark-850 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 hover:text-gray-900 dark:hover:text-white hover:border-slate-500"}`}>
                    ...
                    <div>
                        ...
                        {pending.length === 0 ? (
                            ...
                            <div className="space-y-4">
                                {pending.map((r) => (
                                    <div key={r.id} className={`rounded-2xl bg-white dark:bg-dark-900 p-6 border border-slate-200 dark:border-slate-800 shadow-float border-l-4 ${TYPE_ACCENT[r.type] || "border-l-slate-500"}`}>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                                        ...
                                        <tbody className="divide-y divide-slate-200 dark:divide-slate-800/50">
                                            {processed.map((r) => (
                                                <tr key={r.id} className="even:bg-slate-50/50 dark:even:bg-dark-850/30 hover:bg-gray-50 dark:hover:bg-dark-850/60 transition-colors">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setRejectId(null)}>
                    <div className="w-full max-w-md rounded-3xl bg-white dark:bg-dark-900 p-8 shadow-2xl border border-slate-200 dark:border-slate-800" onClick={(e) => e.stopPropagation()}>
                        ...
                        <textarea rows={4} value={reason} onChange={(e) => setReason(e.target.value)} autoFocus
                            className="block w-full rounded-2xl border border-slate-200 dark:border-slate-700 px-4 py-3 text-sm font-medium text-gray-900 dark:text-white bg-white dark:bg-dark-850 focus:bg-slate-100 dark:focus:bg-dark-900 focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-colors resize-none mb-2" />
                        <div className="mt-6 flex justify-end gap-4">
                            <button onClick={() => setRejectId(null)} className="rounded-xl border border-slate-200 dark:border-slate-700 px-6 py-3 text-sm font-bold text-gray-900 dark:text-white bg-white dark:bg-dark-850 hover:bg-gray-100 dark:hover:bg-dark-800 hover:border-slate-600 transition-colors">Cancel</button>
