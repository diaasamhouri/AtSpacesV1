...
const STATUSES = ["ALL", "PENDING", "PENDING_APPROVAL", "CONFIRMED", "CHECKED_IN", "COMPLETED", "CANCELLED", "REJECTED", "NO_SHOW", "EXPIRED"];
            ...
            <div className="flex flex-wrap gap-2">
                {STATUSES.map((s) => (
                    <button key={s} onClick={() => setFilter(s)}
                        className={`relative rounded-xl px-4 py-2 text-xs font-bold transition-all duration-300 ${filter === s ? "bg-brand-500 text-white shadow-[0_2px_8px_rgba(255,91,4,0.4)]" : "bg-slate-100 dark:bg-dark-850 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 hover:text-gray-900 dark:hover:text-white hover:border-slate-500"}`}>
            ...
            {loading ? (
                ...
                <div className="space-y-4 md:hidden">
                    {bookings.length === 0 ? (
                        ...
                        <div key={b.id} onClick={() => router.push(`/admin/bookings/${b.id}`)} className="rounded-2xl bg-white dark:bg-dark-900 border border-slate-200 dark:border-slate-800 p-5 cursor-pointer hover:border-brand-500/50 transition-colors">
