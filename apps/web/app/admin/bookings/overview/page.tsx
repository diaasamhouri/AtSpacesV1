...
const statusColors: Record<string, string> = {
  ...
  CHECKED_IN: "bg-purple-500/10 text-purple-500",
  COMPLETED: "bg-green-500/10 text-green-500",
  CANCELLED: "bg-red-500/10 text-red-500",
  REJECTED: "bg-red-500/10 text-red-400",
  NO_SHOW: "bg-slate-500/10 text-slate-400",
  EXPIRED: "bg-slate-500/10 text-slate-500",
};

export default function AdminBookingOverviewPage() {
  const { token } = useAuth();
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await getAdminBookings(token, { page, limit: 20, search: search || undefined });
  ...
  const columns: Column<AdminBooking>[] = [
    ...
    },
    {
      header: "Status",
      accessor: (row) => (
        <span className={`text-xs rounded-full px-2 py-0.5 font-medium ${statusColors[row.status] || "bg-slate-500/10 text-slate-400"}`}>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Booking Overview</h1>
      ...
      <DataTable
        ...
        exportable
        exportFilename="admin-all-bookings"
