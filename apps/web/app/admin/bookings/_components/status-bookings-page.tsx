...
export default function AdminStatusBookingsPage({ status, title, emptyMessage, exportFilename }: AdminStatusBookingsPageProps) {
  const { token } = useAuth();
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await getAdminBookings(token, { page, limit: 20, status, search: search || undefined });
